...
class UploadHandler {
  ...
  private function create_thumbnail($file_name) {
    ...
    $scale = min(
            $this->thumbnail_max_width / $img_width, $this->thumbnail_max_height / $img_height
    );
    if ($scale > 1) {
      $scale = 1;
    }
    ...
    $success = $src_img && @imagecopyresampled(
                    $thumbnail_img, $src_img, 0, 0, 0, 0, $thumbnail_width, $thumbnail_height, $img_width, $img_height
  ...
  public function get() {
    ...
    if ($file_name) {
      ...
      $info = array_values(array_filter(array_map(
                              array($this, 'get_file_object'), scandir($this->upload_dir)
      )));
    }
